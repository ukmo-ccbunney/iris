...
"""Module to support the saving of Iris cubes to a Zarr file.

Module to support the saving of Iris cubes to a Zarr file, also using the CF
...
# Get the logger : shared logger for all in 'iris.fileformats.zarr'.
from . import logger
...
CFVariable = typing.Union[
    _thread_safe_nc.VariableWrapper, VariableEmulator, zarr.core.array.Array
]
...
class Saver:
    ...
    def __init__(self, filename, zarr_format=3, compute=True):
        ...
        zarr_format : int
            Underlying Zarr file format, one of 2 or 3 (default).
        ...
        """
        if zarr_format not in [3, 4]:
            raise ValueError("Unknown Zarr file format, got %r" % zarr_format)
        ...
        # Create new Zarr store based on filename
        # TODO(chris): handle the case where someone passes in a Store object
        #       which might already contain data.
    ...
    def __exit__(self, type, value, traceback):
        """Write any delayed data to the zarr file.

        Note that Zarr files do not require "closing" - they are
        synced automatically. We close here though as a precaution!
        """
        # TODO(chris): delayed compute?
        pass
    ...
    def write(
        ...
        """Wrap for saving cubes to a Zarr file.

        Parameters
        ----------
        cube : :class:`iris.cube.Cube`
            A :class:`iris.cube.Cube` to be saved to a Zarr file.
        ...
        zlib : bool, default=False
            If `True`, the data will be compressed in the Zarr file using
        ...
        chunksizes : tuple of int, optional
            Used to manually specify the ZArr chunksizes for each dimension of
        ...
        packing : type or str or dict or list, optional
            ...
            will occur.
        fill_value : optional
            The value to use for the `_FillValue` attribute on the Zarr
        ...
        ## TODO(chris): Dimensions are not a thing in ZArr...can probably just
        # remove any calls to _create_cf_dimensions
        # self._create_cf_dimensions(cube, all_dimensions)
        ...
        # TODO(chris): # cf_mesh_name = self._add_mesh(cube, compression_kwargs=compression_kwargs)

        # Create the associated cube CF-Zarr data variable.
    ...
    def update_global_attributes(self, attributes=None, **kwargs):
        ...
        if attributes is not None:
            ...
            for attr_name in sorted(attributes):
                _set_attr(self._root_group, attr_name, attributes[attr_name])

        for attr_name in sorted(kwargs):
            _set_attr(self._root_group, attr_name, kwargs[attr_name])
    ...
    def _create_cf_dimensions(self, cube, dimension_names):
        ...
        for dim_name in dimension_names:
            ...
            if dim_name not in self._root_group.keys():
        ...
        if mesh:
            ...
            if cf_mesh_name is None:
                ...
                for location in MESH_ELEMENTS:
                    ...
                    if mesh_coords:
                        ...
                        if coord_names:
                            ...
                            _set_attr(cf_mesh_var, coords_file_attr, coord_names)
                ...
                for conn in conns:
                    ...
                    _set_attr(cf_conn_var, "cf_role", cf_conn_attr_name)
                    _set_attr(cf_conn_var, "start_index", conn.start_index)

                    # Record the connectivity on the parent mesh var.
                    _set_attr(cf_mesh_var, cf_conn_attr_name, cf_conn_name)
                    ...
                    if conn.location_axis == 1:
                        ...
                        _set_attr(cf_mesh_var, loc_dim_attr, loc_dim_name)
        ...
        *,
        ...
        if coordlike_elements:
            ...
            if element_names:
                ...
                _set_attr(cf_var_cube, role_attribute_name, variable_names)
    ...
    def _add_aux_factories(self, cube, cf_var_cube, dimension_names):
        ...
        for factory in cube.aux_factories:
            ...
            if factory_defn is None:
                ...
                cf_var = self._root_group[cf_name]
                ...
                if hasattr(cf_var, "formula_terms"):
                    if (
                        ...
                        if name is None:
                            ...
                            cf_var = self._root_group[name]
                            _set_attr(cf_var, "standard_name", std_name)
                            _set_attr(cf_var, "axis", "Z")
                            # Update the formula terms.
                            ft = formula_terms.split()
                            ft = [name if t == cf_name else t for t in ft]
                            _set_attr(cf_var, "formula_terms", " ".join(ft))
                            # Update the cache.
                        ...
                        _set_attr(cf_var_cube, "coordinates", " ".join(coords))
                else:
                    _set_attr(cf_var, "standard_name", std_name)
                    _set_attr(cf_var, "axis", "Z")
                    _set_attr(cf_var, "formula_terms", formula_terms)
                ...
                if FUTURE.derived_bounds:
                    ...
                    bounds_varname = cf_var.attrs.get("bounds", None)
                    cf_bounds_var = self._root_group.get(bounds_varname, None)
                    if (
                        cf_bounds_var is not None
                        and cf_bounds_var.attrs.get("formula_terms", None) is None
                    ):
                        ...
                        def boundsterm_varname(term_varname):
                            ...
                            termvar = self._root_group[term_varname]
                            boundsname = termvar.attrs.get("bounds", None)
                            if boundsname in self._root_group:
                                result = boundsname
                            return result
                        ...
                        _set_attr(cf_bounds_var, "formula_terms", bounds_formula_terms)
                ...
                if len(dim_coords) > 0:
                    ...
                    if dim_name is None:
                        ...
                        while (
                            ...
                            or dim_name
                            in self._root_group.keys()  # TODO: or array_keys(), i.e. not groups?
                        ):
    ...
    def _ensure_valid_dtype(self, values, src_name, src_object):
        ## TODO(chris): Anything needed here?
        return values

    def _create_cf_bounds(self, coord, cf_var, cf_name, /, *, compression_kwargs=None):
        """Create the associated CF bounds variable.
        ...
        if hasattr(coord, "has_bounds") and coord.has_bounds():
            ...
            # TODO(chris): We don't need to (and can't) create `dimensions` in zarr
            ...
            dimension_names = cf_var.metadata.dimension_names + (bounds_dimension_name,)

            _set_attr(cf_var, property_name, boundsvar_name)
            print(
                f"CREATING ZARR ARRAY (BOUNDS): name={boundsvar_name} shape={bounds.shape} {dimension_names=}"
            )
            cf_var_bounds = self._root_group.create_array(
                name=boundsvar_name,
                shape=bounds.shape,
                dtype=bounds.dtype.newbyteorder("="),
    ...
    def _create_mesh(self, mesh):
        ...
        _set_attr(cf_mesh_var, "cf_role", "mesh_topology")
        _set_attr(
            cf_mesh_var,
            "topology_dimension",
    ...
    def _set_cf_var_attributes(self, cf_var, element):
        ...
        if cf_units.as_unit(units_str).is_udunits():
            _set_attr(cf_var, "units", units_str)

        standard_name = element.standard_name
        if standard_name is not None:
            _set_attr(cf_var, "standard_name", standard_name)

        long_name = element.long_name
        if long_name is not None:
            _set_attr(cf_var, "long_name", long_name)

        # Add the CF-netCDF calendar attribute.
        if element.units.calendar:
            _set_attr(cf_var, "calendar", str(element.units.calendar))
    ...
    def _create_generic_cf_array_var(
        ...
        *,
        ...
        """Create the CF variable given dimensional_metadata.

        Create the associated CF variable in the dataset for the
        ...
        -------
        str
            The name of the CF variable created.
        """
        # Support cube or mesh save.
        from iris.cube import Cube
        ...
        while cf_name in self._root_group.keys():
        ...
        if False and np.issubdtype(data.dtype, np.str_):
            # Deal with string-type variables.
            # Typically CF label variables, but also possibly ancil-vars ?

            # NOTE: Zarr doesn't need any special handling of strings as
            # it supports dtypes like 'U4' which have the string length built in

            # Determine whether to create the string length dimension.
            # TODO(chris): byte strings are technically not supported by the Zarr spec
            # They DO work, but are subject to change. Also, we don't need a
            # separate dimension for the string length - this can be passed
            # in the dtype, e.g. 'S4'.
            ...
            ## If shape is (1,) and we have no associated dimensions, then this is
            # a scalar variable and needs to be created with shape=() so readers (e.g Xarray)
            # will not try to associate ir with an (unknown) dimensions.
            shape = () if data.shape == (1,) and len(element_dims) == 0 else data.shape
            print(
                f"CREATING ZARR ARRAY (COMPONENT): name={cf_name} {shape=} dimension_names={element_dims}"
            )
            cf_var = self._root_group.create_array(
                name=cf_name,
                shape=shape,
                dtype=data.dtype.newbyteorder("="),
                dimension_names=element_dims,
                fill_value=fill_value,
                # **compression_kwargs, ## TODO
            )
            ...
            if is_dimcoord:
                ...
                if axis is not None and axis.lower() in SPATIO_TEMPORAL_AXES:
                    _set_attr(cf_var, "axis", axis.upper())
    ...
    def _add_grid_mapping_to_dataset(self, cs, extended_grid_mapping=False):
        """Create a CF grid mapping variable and add to the dataset.
        ...
        """
        print(
            f"CREATING ZARR ARRAY (GRID_MAPPING): name={cs.grid_mapping_name} shape=() dimension_names=()"
        )
        cf_var_grid = self._root_group.create_array(
            name=cs.grid_mapping_name, dtype=np.int32, shape=()
        )
        _set_attr(cf_var_grid, "grid_mapping_name", cs.grid_mapping_name)
        ...
        def add_ellipsoid(ellipsoid):
            ...
            if semi_minor == semi_major:
                _set_attr(cf_var_grid, "earth_radius", semi_major)
            else:
                _set_attr(cf_var_grid, "semi_major_axis", semi_major)
                _set_attr(cf_var_grid, "semi_minor_axis", semi_minor)
            if ellipsoid.datum is not None:
                _set_attr(cf_var_grid, "horizontal_datum_name", ellipsoid.datum)
        ...
        # tmerc
            ...
            _set_attr(cf_var_grid, "false_easting", cs.false_easting)
            _set_attr(cf_var_grid, "false_northing", cs.false_northing)
            _set_attr(
                cf_var_grid,
        ...
        # merc
            ...
            if cs.standard_parallel is not None:
                _set_attr(cf_var_grid, "standard_parallel", cs.standard_parallel)
        ...
        # lcc
            ...
            if cs.ellipsoid:
                add_ellipsoid(cs.ellipsoid)
            _set_attr(cf_var_grid, "standard_parallel", cs.secant_latitudes)
            _set_attr(cf_var_grid, "latitude_of_projection_origin", cs.central_lat)
            _set_attr(cf_var_grid, "longitude_of_central_meridian", cs.central_lon)
            _set_attr(cf_var_grid, "false_easting", cs.false_easting)
            _set_attr(cf_var_grid, "false_northing", cs.false_northing)
        ...
        # polar stereo (have to do this before Stereographic because it subclasses it)
            ...
            if cs.true_scale_lat is not None:
                _set_attr(cf_var_grid, "true_scale_lat", cs.true_scale_lat)
            elif cs.scale_factor_at_projection_origin is not None:
                _set_attr(
                    cf_var_grid,
                    "scale_factor_at_projection_origin",
                ...
                _set_attr(cf_var_grid, "scale_factor_at_projection_origin", 1.0)

        # stereo
        elif isinstance(cs, iris.coord_systems.Stereographic):
            if cs.ellipsoid:
                add_ellipsoid(cs.ellipsoid)
            _set_attr(cf_var_grid, "longitude_of_projection_origin", cs.central_lon)
            _set_attr(cf_var_grid, "latitude_of_projection_origin", cs.central_lat)
            _set_attr(cf_var_grid, "false_easting", cs.false_easting)
            _set_attr(cf_var_grid, "false_northing", cs.false_northing)
            # Only one of these should be set
        ...
        # albers conical equal area
            ...
            _set_attr(cf_var_grid, "false_easting", cs.false_easting)
            _set_attr(cf_var_grid, "false_northing", cs.false_northing)
            _set_attr(cf_var_grid, "standard_parallel", cs.standard_parallels)
        ...
        # geostationary
            ...
            _set_attr(cf_var_grid, "sweep_angle_axis", cs.sweep_angle_axis)
    ...
    def _create_cf_grid_mapping(self, cube, cf_var_cube):
        """Create CF grid mapping and associated CF variable.

        Create CF grid mapping variable and associated CF data
        variable grid mapping attribute.
        ...
        for cs in coord_systems:
            ...
            if cs not in self._coord_systems:
                ...
                while cs.grid_mapping_name in self._root_group.keys():
        ...
        if len(coord_systems):
            ...
            if grid_mapping:
                _set_attr(cf_var_cube, "grid_mapping", grid_mapping)
    ...
    def _create_cf_data_variable(
        ...
        **kwargs,
        ...
        """Create Zarr array for the cube data and any associated grid mapping.

        Parameters
        ----------
        cube : :class:`iris.cube.Cube`
            The associated cube being saved to Zarr file.
        dimension_names : list
        ...
        def set_packing_ncattrs(cfvar):
            ...
            # TODO(chris): Thread safe needed in zarr?
            # assert hasattr(cfvar, "THREAD_SAFE_FLAG")
            if packing:
                if scale_factor:
                    _set_attr(cfvar, "scale_factor", scale_factor)
                if add_offset:
                    _set_attr(cfvar, "add_offset", add_offset)

        cf_name = self._get_cube_variable_name(cube)
        while cf_name in self._root_group.array_keys():
            cf_name = self._increment_name(cf_name)

        # Create the Zarr array for the cube data variable with data payload.
        # TODO(chris): Handle **kwargs for compression, chunking, etc
        print(
            f"CREATING ZARR ARRAY (DATA VARIABLE): name={cf_name} shape={cube.shape} {dimension_names=}"
        )
        cf_var = self._root_group.create_array(
            name=cf_name,
            dtype=dtype,
            ...
            dimension_names=dimension_names,
            fill_value=fill_value,
            # TODO Handle other keyword args: **kwargs
        )
        ...
        if cube.standard_name:
            _set_attr(cf_var, "standard_name", cube.standard_name)

        if cube.long_name:
            _set_attr(cf_var, "long_name", cube.long_name)

        if cube.units.is_udunits():
            _set_attr(cf_var, "units", str(cube.units))

        # Add the CF-netCDF calendar attribute.
        if cube.units.calendar:
            _set_attr(cf_var, "calendar", cube.units.calendar)
    ...
    def _lazy_stream_data(
        ...
        # TODO(chris): NEED TO HANDLE LAZY DATA FOR ZARR
    ...
    def complete(self) -> None:
        ...
        """
        # TODO(chris) - implement for zarr delayed save
...
def save(
    ...
    * The save process will stream the data payload to the file using dask,
    ...
    with Saver(filename, zarr_format, compute=compute) as sman:
