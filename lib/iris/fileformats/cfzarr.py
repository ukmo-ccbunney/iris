...
class CFVariable(metaclass=ABCMeta):
    ...
    def __getattr__(self, name):
        ...
        # special case for `dimensions` : this is a property of an array in zarr and
        # is instead provided as a metadata value:
        if name == "dimensions":
            if self.cf_data.metadata.zarr_format > 2:
                value = self.cf_data.metadata.to_dict().get("dimension_name", ())
            else:
...
class CFCoordinateVariable(CFVariable):
    ...
    def identify(cls, variables, ignore=None, target=None, warn=True, monotonic=False):
        ...
        for nc_var_name, nc_var in target.items():
            ...
            # NOTE[ChrisB]: For Zarr, there are no dimension objects, so we check that the
            # variable has exactly one value in the `dimension_name` metadata and that the
            # name of the variable is the same as the name of its  single dimension.
            dims = (
                nc_var.metadata.to_dict().get("dimension_name", [])
                if nc_var.metadata.zarr_format > 2
                else nc_var.attrs.get("_ARRAY_DIMENSIONS", [])
            )
            if not (nc_var.ndim == 1 and nc_var_name in dims):
                continue
    ...
    * [CF] Appendix F. Grid Mappings.
    ...
    def identify(
        ...
        for nc_var_name, nc_var in target.items():
            ...
            # nc_var_att = getattr(nc_var, cls.cf_identity, None)
            nc_var_att = nc_var.attrs.get(cls.cf_identity, None)

            if nc_var_att is not None:
                print(f"Got Grid mapping var for {nc_var_name}: {nc_var_att}")
...
class CFReader:
    ...
    def __init__(self, file_source, warn=False, monotonic=False):
        ...
        # Convert list of tuples to dict as this is what the rest of the code expects.
        variables = dict(self._dataset.members())
    ...
    def _has_meshes(self):
        result = False
        for variable in self._dataset.array_values():
            if "mesh" in variable.attrs or "node_coordinates" in variable.attrs:
                result = True
                break
        return result
    ...
    def _translate(self, variables):
        ...
        for nc_var in variables.values():
            if grid_mapping_attr := nc_var.attrs.get("grid_mapping", None):
                try:
        ...
        # TODO: I don't understand why we are doing this here dcict comprehension...
        # Can't we just pass the dataset.attrs directly to `self.cf_group.global_attributes.update`
        ## attr_dict = {
        ##     attr_name: self._dataset.attrs.get(attr_name, "")
        ##     for attr_name in self._dataset.attrs.keys()
        ## }
        ##self.cf_group.global_attributes.update(attr_dict)
        self.cf_group.global_attributes.update(self._dataset.attrs)
    ...
    def _close(self):
        ...
        # This is important for ZArr ZipStores stores (which must be explicitly close),
        #  but less so for normal LocalStores
        if self._own_file and self._dataset is not None:
            self._dataset.store.close()
            self._dataset = None
